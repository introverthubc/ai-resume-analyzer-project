...
export function ResumeDocument({ user, version, title }) {
  // Export the selected version verbatim. Do not replace the candidate's name
  // with the signed-in demo profile or omit sections we cannot classify.
  if (version?.text) return (
    <Document title={title || "Resume"}>
      <Page size="A4" style={styles.page}>
        {version.text.split('\n').map((line, i) => (
          <Text key={i} style={i === 0 ? styles.name : styles.summary}>{line || ' '}</Text>
        ))}
