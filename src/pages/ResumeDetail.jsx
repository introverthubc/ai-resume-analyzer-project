...
export default function ResumeDetail() {
  ...
  const activeVersionId = selection?.resumeId === id ? selection.versionId : resume?.currentVersionId || versions.at(-1)?._id;
      ...
      if (res?.version?._id) {
        ...
        try {
          await analyze.mutateAsync({
            ...
            targetRole: analysis.targetRole || targetRole.trim() || undefined,
          });
        } catch {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-4 justify-between">
          ...
          <div className="flex items-center gap-3 flex-1 min-w-[280px] max-w-[520px]">
            ...
            <Button
              ...
              size="lg"
              onClick={runAnalyze}
              disabled={analyze.isPending || applyRewrites.isPending || !activeVersionId}
              className="shrink-0"
            >
              {analyze.isPending ? (
                <>
      ...
      {analysisQuery.error && (
        <EmptyState icon={FileText} title="Couldn't load analysis" description={analysisQuery.error.message} />
      )}
      {!analysis && !analysisQuery.isLoading && !analysisQuery.error && (
        <EmptyState
          icon={Sparkles}
            ...
            <div className="mt-5">
              ...
              <TabsContent value="rewrites">
                <BulletRewrites
                  ...
                  isApplying={applyRewrites.isPending || analyze.isPending}
                  error={applyRewrites.error?.message}
                />
              </TabsContent>
              <TabsContent value="diff">
                <DiffView key={`${id}:${versions.length}`} resumeId={id} versions={versions} />
              </TabsContent>
            </div>
          </Tabs>
        </>
      ...
      {activeVersion && (
        <Card>
          <CardHeader>
            <div>
              <CardTitle className="text-base">Resume text ({activeVersion.label})</CardTitle>
              <CardDescription className="mt-1">
                Full text of this version, including any applied rewrites
              </CardDescription>
            </div>
          </CardHeader>
...
function ParsedSectionsPreview({ version }) {
  if (version.text) return <div className="whitespace-pre-wrap text-sm text-[var(--ink)] leading-relaxed max-h-[600px] overflow-auto">{version.text}</div>;
